...
object LogisticPredict {
    ...
    // 解决：1. 增加spark.executor.memory=7000M的内存和spark.driver.memory=6000M的内存（可直接在spark-env.sh中修改）
    // 2. 在VM options中增加："-XX:-UseGCOverheadLimit" 取消GC占用大量时间为释放很小空间时的保护机制，参考：http://java-boy.iteye.com/blog/463454
    ...
    /**
      ...
      * 结果=> 预测的结果量太少，只有不到50条记录。TODO 进一步优化
      * ----------------------
      * TODO step 4
      * 自变量加入用户位置的空间标识
      * ----------------------
